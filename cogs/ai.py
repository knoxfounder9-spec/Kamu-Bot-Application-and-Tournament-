...
class AICog(commands.Cog):
    ...
    def is_nsfw(self, text):
        nsfw_words = ['nsfw', 'porn', 'nude', 'sex', 'boobs', 'dick', 'vagina', 'hentai', 'rule34', 'naked', 'gore', 'pussy', 'cock', 'tit', 'asshole', 'cum', 'slut', 'whore']
    ...
    @app_commands.command(name="imagine", description="Generate an image using premium AI models")
    @app_commands.describe(prompt="What do you want to generate?")
    async def imagine(self, interaction: discord.Interaction, prompt: str):
        if self.is_nsfw(prompt):
            await interaction.response.send_message("🚫 NSFW requests are automatically cancelled.", ephemeral=True)
            return
        
        await interaction.response.defer()
        
        encoded_prompt = urllib.parse.quote(prompt)
        # safe=true enables pollinations' built-in NSFW filter as a backup
        image_url = f"https://image.pollinations.ai/prompt/{encoded_prompt}?nologo=true&safe=true"
        
        embed = discord.Embed(title="🎨 Image Generated", description=f"**Prompt:** {prompt}", color=discord.Color.purple())
        embed.set_image(url=image_url)
        embed.set_footer(text="Generated by Premium AI")
    ...
    async def process_queue(self, channel_id):
        ...
        try:
            while channel_id in self.channel_queues and self.channel_queues[channel_id]:
                message = self.channel_queues[channel_id].pop(0)
                await self.handle_ai_message(message)
        finally:
            self.processing.remove(channel_id)
            if channel_id in self.channel_queues and not self.channel_queues[channel_id]:
    ...
    async def on_message(self, message: discord.Message):
        ...
        if is_enabled and message.channel.id == channel_id:
            ...
            self.channel_queues[channel_id].append(message)
            
            # Start processing the queue if not already running
            self.bot.loop.create_task(self.process_queue(channel_id))
    ...
    async def handle_ai_message(self, message: discord.Message):
        ...
        # Wait in queue (FIFO) if multiple people chat at the same time
        async with self.ai_lock:
            async with message.channel.typing():
                # Inject a hidden system prompt to force the AI to act like a paid/premium service
                premium_instruction = "\n\nCRITICAL INSTRUCTION: You are a premium, paid, enterprise-grade AI. You must NEVER mention being free, open-source, or using free providers. If asked about your cost, model, or nature, state that you are a highly advanced, paid premium AI service. You must NEVER use @everyone, @here, or ping any roles. If the user asks you to generate, draw, or create an image, reply EXACTLY with '[GENERATE_IMAGE: <describe the image in detail>]' and no other text."
                ...
                if "[GENERATE_IMAGE:" in response:
                    match = re.search(r'\[GENERATE_IMAGE:\s*(.*?)\]', response, re.IGNORECASE | re.DOTALL)
                    if match:
                        image_prompt = match.group(1).strip()
                        if self.is_nsfw(image_prompt) or self.is_nsfw(message.content):
                            await message.reply("🚫 NSFW image requests are automatically cancelled.")
                            return
                        
                        encoded_prompt = urllib.parse.quote(image_prompt)
                        image_url = f"https://image.pollinations.ai/prompt/{encoded_prompt}?nologo=true&safe=true"
                        
                        embed = discord.Embed(title="🎨 Image Generated", description=f"**Prompt:** {image_prompt}", color=discord.Color.purple())
                        embed.set_image(url=image_url)
                        embed.set_footer(text="Generated by Premium AI")
                        ...
                        self.add_memory(message.author.id, message.guild.id, "assistant", f"Generated image: {image_prompt}")
                        return
